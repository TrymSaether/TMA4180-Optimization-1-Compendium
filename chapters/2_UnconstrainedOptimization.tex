...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% Initial simplex (blue solid)
		\draw[blue, thick]
		(1,1,1.5) -- (2,1,1.0) -- (1.5,2,1.3) -- cycle;

		% Reflected simplex (red dashed)
		\draw[red, thick, dashed]
		(0.5,1,1.2) -- (1.8,1,0.9) -- (1.1,2.1,1.0) -- cycle;

		% Contracted simplex (green dotted)
		\draw[green!50!black, thick, dotted]
		(1.2,1.1,1.1) -- (1.6,1.0,1.0) -- (1.3,1.7,1.05) -- cycle;

	\end{tikzpicture}

	\vspace{1em}
	\begin{tikzpicture}[scale=1]
		% Manual legend below the 3D figure
		\draw[blue, thick] (0,1.2) -- (0.8,1.2) node[right, black] {Initial simplex};
		\draw[red, thick, dashed] (0,0.6) -- (0.8,0.6) node[right, black] {Reflected simplex};
		\draw[green!50!black, thick, dotted] (0,0) -- (0.8,0) node[right, black] {Contracted simplex};
	\end{tikzpicture}

	\caption{3D-visualisering av Nelder--Mead-algoritmen: blå trekant viser initialt simplex, rød viser refleksjon og grønn viser kontraksjon. Nivålinjene på basen representerer nivåsett av objektfunksjonen.}
...
% Trust Region metoden er i motsetning til linjemetoden en tilnærming som bestemmer både retning og lengde samtidig. 
% Den definerer et tillitsområde rundt den nåværende iterasjonen innenfor hvilket den antar at den kvadratiske modellen er en tilstrekkelig representasjon av objektfunksjonen.
% I trust region-metoden løser vi underproblemet:
% \[
...
% \]
% hvor $f_k = f(\symbf{x}_k)$, $\symbf{g}_k = \nabla f(\symbf{x}_k)$, og $\symbf{B}_k$ er en symmetrisk matrise som approksimerer Hesse-matrisen $\nabla^2 f(\symbf{x}_k)$.
